              ...
              <div class="tab-content">
                <div class="tab-pane fade show active" id="home">
                  ...
                  <div class="input-group">
                      <input type="text" v-model="tnumber" class="form-control custom-input" placeholder="Enter Your Tracking Number" aria-label="Recipient's username">
                      <div class="input-group-append">
                        <button @click="Track" class="btn btn-sm btn-primary custom-btn" type="button">Track Now</button>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <form method="POST" id="accountCreatee" action="/userapi/create-account" class="forms-sample">
                    <div id="sender-info">
                      ...
                      <div class="form-group">
                        ...
                        <select v-model="s_destination" name="country" class="form-control form-control-lg" id="exampleFormControlSelect2">
                    ...
                    <div style="display: none;" id="other-info">
                      ...
                      <div class="row">
                        ...
                        <div class="col-12">
                          ...
                          <button type="button"  @click="Verify" class="btn btn-success mr-2">Quote</button>
...
<script>
  var app = new Vue({
    ...
    data() {
      return {
        s_forename: '<?php echo $_SESSION["nameofcompany"] ?>',
    ...
    methods: {
      Track() {
        axios.get('../../api/order/view_order_by_tracking_api.php?track=' + this.tnumber).then(response => {
          ...
          if (response.data.code == 0) {
            ...
            Notiflix.Report.Info(
              'Order Details',
              'Find Status of Your Order Below .........<br/><br/>'+response.data[0].order_status,
              'Okay',
            );
      ...
      Verify() {
        if (this.r_email == "" || this.r_forename == "" || this.r_addressone == "" || this.r_city == "" || this.r_state == "" || this.r_country == "" || this.r_contact == "") {
          Notiflix.Report.Failure(
            'Error',
            '"Receiver Details not Complete"',
            'Okay',
          );
        } else if (this.p_forename == "" || this.p_addressone == "" || this.p_city == "" || this.p_state == "" || this.p_country == "" || this.p_contact == "") {
          Notiflix.Report.Failure(
            'Error',
            '"Pick Up Details not Complete"',
            'Okay',
          );
        } else if (this.deliverymethod == "Select Delivery Method") {
          Notiflix.Report.Failure(
            'Error',
          ...
          Notiflix.Report.Failure(
            'Error',
            '"Contact Administrator For this Order"',
            'Okay',
          );
        } else {
      ...
      SendToServer() {
        ...
        axios({
            method: "post",
            url: "../../api/order/create_order_api.php",
            data: bodyFormData,
          })
          .then(function(response) {
            ...
            if (response.data.code == 1) {
              Notiflix.Report.Success(
                'Success',
                '"Order Placed in Successfully"',
                'Close',
              );
              setTimeout(function() {
                window.location.href = 'summary?order_id='+response.data.order_id;
