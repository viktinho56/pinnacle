...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['nameofcompany']) && !empty($_POST['maincontactperson']) && !empty($_POST['addressone'])  && !empty($_POST['city']) && !empty($_POST['state']) && !empty($_POST['country']) &&  !empty($_POST['email']) && !empty($_POST['contactnumber'])  && !empty($_POST['avatar']) && !empty($_POST['p1']) &&!empty($_POST['p2']) &&!empty($_POST['p3']) &&!empty($_POST['p4'])) {
	    ...
	    if ($email === false) {
            ...
            $pin = $p1.$p2.$p3.$p4;
            $businessobj->business_profile_update($email,$nameofcompany, $maincontactperson, $companytype, $typeofcompany,$adressone,$addresstwo,$city,$state,$country,$postcode,$contactnumber,$avatar,$notification,$pin);
        }
