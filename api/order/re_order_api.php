...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $orderobj = new order();
    if (
        !empty($_POST['s_destination']) && !empty($_POST['s_forename']) && !empty($_POST['s_addressone'])  && 
	    ...
	    if ($s_email === false || $r_email ===false) {
            ...
            $orderobj->re_order($s_destination,$s_forename,$s_addresslineone,$s_addresslinetwo,$s_city,$s_state,$s_country,$s_postcode,$r_forename,$r_contact,$r_addresslineone,$r_addresslinetwo,$r_city,$r_state,$r_country,$r_postcode,$p_destination,$p_forename,$p_addresslineone,$p_addresslinetwo,$p_city,$p_state,$p_country,$p_postcode,$deliverymethod,$seafreight,$airfreight,$weight,$items,$items_desc,$amount,$s_email,$r_email,$s_contact);
        }
