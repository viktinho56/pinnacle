...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $userobj = new admin();
    if (!empty($_POST['forenames']) && !empty($_POST['surname']) && !empty($_POST['addressone'])  && !empty($_POST['city']) && !empty($_POST['state']) && !empty($_POST['country']) &&  !empty($_POST['email']) && !empty($_POST['contactnumber']) ) {
      ...
      //  $notification = trim(htmlspecialchars($_POST['notification']));
	    ...
	    if ($email === false) {
            ...
            $userobj->UserProfileUpdate($email,$forenames,$surname,$adressone,$addresstwo,$city,$state,$country,$contactnumber);
